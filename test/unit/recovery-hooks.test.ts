...
function createRecoveryRun(ownerSessionId = "session-a"): { cwd: string; runId: string; manifest: ReturnType<typeof createRunManifest>["manifest"] } {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pi-crew-recovery-"));
	fs.mkdirSync(path.join(cwd, ".crew"), { recursive: true });
	const team = { name: "recovery", description: "", roles: [{ name: "worker", agent: "worker" }], source: "test", filePath: "builtin" } as never;
	const workflow = { name: "wf", description: "", steps: [{ id: "one", role: "worker" }], source: "test", filePath: "builtin" } as never;
	const created = createRunManifest({ cwd, team, workflow, goal: "recovery-test", ownerSessionId });
...
describe("run_recovery hook", () => {
	...
	it("allows recovery when hook outcome is allow", async () => {
		...
		try {
			...
			registerHook({ name: "run_recovery", mode: "non_blocking", handler: async () => ({ outcome: "allow" as const }) });
			await applyRecoveryPlan({ runId: run.runId, resumableTasks: ["task-1"], preservedTasks: [], lastEventSeq: 0 }, { cwd: run.cwd });
	...
	it("blocks recovery when hook outcome is block", async () => {
		...
		try {
			saveRunTasks(run.manifest, [{ id: "task-1", runId: run.runId, role: "worker", agent: "worker", title: "task", status: "running", dependsOn: [], cwd: run.cwd }]);
			registerHook({ name: "run_recovery", mode: "blocking", handler: async () => ({ outcome: "block" as const, reason: "Maintenance window" }) });
			await applyRecoveryPlan({ runId: run.runId, resumableTasks: ["task-1"], preservedTasks: [], lastEventSeq: 0 }, { cwd: run.cwd });
			const loaded = loadRunManifestById(run.cwd, run.runId);
...
describe("declineRecoveryPlan", () => {
	it("marks run as cancelled when declined", () => {
		...
		try {
			saveRunTasks(run.manifest, [{ id: "task-1", runId: run.runId, role: "worker", agent: "worker", title: "task", status: "running", dependsOn: [], cwd: run.cwd }]);
			declineRecoveryPlan({ runId: run.runId, resumableTasks: ["task-1"], preservedTasks: [], lastEventSeq: 5 }, { cwd: run.cwd });
			const loaded = loadRunManifestById(run.cwd, run.runId);
