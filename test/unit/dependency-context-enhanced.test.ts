...
const team: TeamConfig = { name: "test-team", description: "test", source: "builtin", filePath: "test.team.md", roles: [{ name: "explorer", agent: "explorer" }, { name: "executor", agent: "executor" }] };
...
test("structured results extracted from JSON result output", () => {
	...
	try {
		...
		const { manifest, tasks } = createRunManifest({ cwd, team, goal: "structured" });
		const jsonContent = JSON.stringify({ files: ["a.ts", "b.ts"], summary: "found 2 files" });
		const resultArtifact = writeArtifact(manifest.artifactsRoot, { kind: "result", relativePath: "results/01_explore.json", producer: "01_explore", content: jsonContent });
		const { explorer, executor } = makeTasks(manifest.runId, cwd, { resultArtifact });
		...
		assert.deepEqual(dep.structuredResults, { files: ["a.ts", "b.ts"], summary: "found 2 files" });
...
test("structured results undefined for non-JSON output", () => {
	...
	try {
		...
		const resultArtifact = writeArtifact(manifest.artifactsRoot, { kind: "result", relativePath: "results/01_explore.md", producer: "01_explore", content: "# Markdown output\n\nSome text." });
		const { explorer, executor } = makeTasks(manifest.runId, cwd, { resultArtifact });
		const step = { id: "execute", role: "executor", task: "do" };
		const ctx = collectDependencyOutputContext({ ...manifest }, [explorer, executor], executor, step);
...
test("artifacts produced lists task artifacts from manifest", () => {
	...
	try {
		fs.mkdirSync(path.join(cwd, ".crew"), { recursive: true });
		const { manifest } = createRunManifest({ cwd, team, goal: "artifacts" });
		const resultArtifact = writeArtifact(manifest.artifactsRoot, { kind: "result", relativePath: "results/01_explore.txt", producer: "01_explore", content: "done" });
		const logArtifact = writeArtifact(manifest.artifactsRoot, { kind: "log", relativePath: "logs/01_explore.log", producer: "01_explore", content: "log output" });
		const updatedManifest = { ...manifest, artifacts: [resultArtifact, logArtifact] };
		const { explorer, executor } = makeTasks(manifest.runId, cwd, { resultArtifact });
		const step = { id: "execute", role: "executor", task: "do" };
...
test("backward compatible — entries without new fields produce undefined", () => {
	...
	try {
		fs.mkdirSync(path.join(cwd, ".crew"), { recursive: true });
		const { manifest } = createRunManifest({ cwd, team, goal: "backcompat" });
		const { explorer, executor } = makeTasks(manifest.runId, cwd);
...
test("render includes structured results, artifacts, and usage when present", () => {
	...
	const rendered = renderDependencyOutputContext({ dependencies: [entry], sharedReads: [] });
